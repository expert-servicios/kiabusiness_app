...
const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Conocimiento Academy", href: "/admin/academy-conocimiento" },
    ],
  },
  {
    label: "Marketing",
    ...
    icon: Zap,
    items: [
      { label: "Contactos EXPERT", href: "/admin/leads" },
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-2.5 border-b border-white/8 px-4 py-4">
        ...
        {isMobile && (
          <button type="button" title="Cerrar menú" onClick={onCloseMobile} className="ml-auto rounded-lg p-1 text-white/40 hover:bg-white/8 hover:text-white">
            <X className="h-4 w-4" />
          </button>
        )}
      </div>
      ...
      <div className="px-3 pt-2">
        <Link
          ...
          className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-semibold transition ${
            isActive("/admin") ? "bg-[#D4A017]/15 text-[#D4A017]" : "text-white/70 hover:bg-white/6 hover:text-white"
          }`}
        >
          <LayoutDashboard className="h-4 w-4 shrink-0" />
          Dashboard
          {urgentCount > 0 && (
            <span className="ml-auto flex h-5 min-w-[20px] items-center justify-center rounded-full bg-amber-500 px-1.5 text-[10px] font-bold text-white">{urgentCount}</span>
          )}
        </Link>
      </div>
          ...
          return (
            <button
              ...
              className={`flex flex-1 flex-col items-center gap-0.5 rounded-lg py-1.5 transition ${
                isSelected ? "bg-[#D4A017]/20 text-[#D4A017]" : hasActive ? "text-[#D4A017]/70 hover:text-[#D4A017]" : "text-white/35 hover:text-white/70"
              }`}
            >
      ...
      <div className="px-4 pt-3 pb-1">
        <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-white/40">{activeGroup.label}</span>
      </div>
        ...
        {activeGroup.items.map((item) => (
          <Link
            ...
            className={`flex items-center justify-between rounded-lg px-3 py-2 text-sm transition ${
              isActive(item.href) ? "bg-[#D4A017]/12 font-semibold text-[#D4A017]" : "text-white/65 hover:bg-white/5 hover:text-white"
            }`}
          >
            {item.label}
            {item.badge ? (
              <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-[#D4A017]/20 px-1.5 text-[10px] font-bold text-[#D4A017]">{item.badge}</span>
            ) : null}
          </Link>
        ))}
      </nav>
      ...
      <div className="border-t border-white/8 px-3 py-3">
        <button type="button" onClick={onOpenDrawer} className="flex w-full items-center gap-2.5 rounded-xl bg-white/4 px-3 py-2.5 text-left transition hover:bg-white/8">
    ...
    <>
      <div className="fixed inset-x-0 top-0 z-50 flex items-center justify-between border-b border-white/8 bg-[#07111d] px-4 py-3 lg:hidden">
        <div className="flex items-center gap-2">
          ...
          {urgentCount > 0 && <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-amber-500 px-1.5 text-[10px] font-bold text-white">{urgentCount}</span>}
        </div>
        <div className="flex items-center gap-2">
          <button type="button" title="Ajustes de cuenta" onClick={() => setDrawerOpen(true)} className="rounded-lg p-1.5 text-white/60 hover:bg-white/8 hover:text-white">
            <User className="h-4 w-4" />
          </button>
          <button type="button" onClick={() => setMobileOpen((o) => !o)} className="rounded-lg p-1.5 text-white/60 hover:bg-white/8 hover:text-white">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#07111d] transition-transform duration-300 lg:hidden ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <AdminUserDrawer open={drawerOpen} onClose={() => setDrawerOpen(false)} userName={userName} userEmail={userEmail} />
    </>
  );
}
