  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 rounded-xl border border-[#D4A017]/30 bg-[#F8F6F1] p-4">
      ...
      <label className="block">
        ...
        <input type="tel" inputMode="tel" pattern="(?=.*[0-9])[+]?[0-9\s().-]{7,20}" title="Introduce un teléfono válido (7-20 dígitos, puede incluir +, espacios o guiones)" value={phone} onChange={(e) => setPhone(e.target.value)} required disabled={loadingProfile} className={inputCls} placeholder="+34 6XX XXX XXX" />
      </label>
