...
const HOLDED_CONTEXT_RE  = /\bholded\b|pack starter|migraci[oó]n holded|formaci[oó]n holded|plan supervision|plan avanzado|plan colaborativo|erp|control horario.*holded|holded.*control horario|холдед/i;
const AEAT_CONTEXT_RE    = /\b(irpf|renta|iva|hacienda|aeat|agencia tributaria|modelo\s*\d{2,3}|tributar|declaraci[oó]n.*renta|impuesto.*renta|fiscal|036|037|130|131|303|390|720|151|beckham|verifactu|no residente|irnr|renta web|campan.*renta)\b/i;
const SS_CONTEXT_RE      = /\b(seguridad social|reta|cotizaci[oó]n|cuota.*aut[oó]nom|aut[oó]nom.*cuota|vida laboral|importass|cese de actividad|tarifa plana|cuota reducida|baja.*laboral|alta.*aut[oó]nom|aut[oó]nom.*alta|inss|tgss|prestaci[oó]n.*aut[oó]nom|aut[oó]nom.*prestaci[oó]n)\b/i;
const DGT_CONTEXT_RE     = /\b(dgt|trafico|transferencia.*vehiculo|vehiculo.*transferencia|matriculacion|canje.*permiso|permiso.*conducir|puntos.*carnet|carnet.*puntos|baja.*vehiculo|multa.*trafico|permiso de circulacion|ficha tecnica|capitania)\b/i;
const JUSTICIA_CONTEXT_RE= /\b(antecedentes penales|registro civil|apostilla|certificado.*nacimiento|nacimiento.*certificado|certificado.*matrimonio|denominacion social|nota simple|registro.*propiedad|registro.*mercantil|deposito.*cuentas|rmc)\b/i;
const PAE_CONTEXT_RE     = /\b(pae|circe|crear empresa online|sl.*online|online.*sl|alta autonomo.*online|online.*alta autonomo|ventanilla unica|constitucion.*online|online.*constitucion)\b/i;
const CCAA_CONTEXT_RE    = /\b(itp|transmisiones patrimoniales|isd|sucesiones|donaciones|ajd|actos juridicos documentados|impuesto.*herencia|herencia.*impuesto|impuesto de patrimonio|plusvalia.*municipal|iivtnu|suma.*alicante|atv.*valencia|hacienda.*comunidad|ccaa.*impuesto|impuesto.*regional)\b/i;

function matchesContext(re: RegExp, params: { currentPage?: string; currentTask?: string; pageData?: Record<string, unknown> }): boolean {
  const text = [params.currentPage ?? '', params.currentTask ?? '', JSON.stringify(params.pageData ?? {})].join(' ');
  return re.test(text);
}
...
export function buildKiaSystemPrompt(params: {
  ...
  const withHolded  = params.includeHolded  ?? matchesContext(HOLDED_CONTEXT_RE,   params);
  const withAeat    = params.includeAeat    ?? matchesContext(AEAT_CONTEXT_RE,     params);
  ...
  const withJusticia= params.includeJusticia?? matchesContext(JUSTICIA_CONTEXT_RE, params);
  const withPae     = params.includePae     ?? matchesContext(PAE_CONTEXT_RE,      params);
  const withCcaa    = params.includeCcaa    ?? matchesContext(CCAA_CONTEXT_RE,     params);
