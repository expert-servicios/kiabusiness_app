...
```mermaid
flowchart TD
    A[Landing del curso] --> B[Pago Stripe]
    A --> C[Formulario de interés]
    A --> D[Reunión informativa]
    A --> E[Descarga del programa]
    C --> F[Seguimiento comercial]
    D --> F
    E --> C
    B --> G[Alta del alumno]
    G --> H[Acceso a manuales privados]
```
