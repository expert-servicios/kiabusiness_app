...
test('OAuth metadata returns canonical HTTPS URLs', async () => {
  ...
  try {
    ...
    assert.equal(body.issuer, 'https://claude.expertconsulting.es');
    assert.equal(body.authorization_endpoint, 'https://claude.expertconsulting.es/oauth/authorize');
    assert.equal(body.token_endpoint, 'https://claude.expertconsulting.es/oauth/token');
    assert.equal(body.registration_endpoint, 'https://claude.expertconsulting.es/oauth/register');
    assert.equal(body.revocation_endpoint, 'https://claude.expertconsulting.es/oauth/revoke');
  } finally {
    await runtime.close();
  }
});
