 ...
 * EXPERT:
 *   POST {EXPERT_APP_URL}/api/integrations/holded/connector-event
 *
...
export async function dispatchConnectorEvent(event: ConnectorEvent): Promise<void> {
  if (config.EXPERT_CENTRAL_REGISTRY_ENABLED !== '1') {
    logger.debug('connector-event skipped: EXPERT_CENTRAL_REGISTRY_ENABLED is disabled');
    return;
  }

  const url = new URL(ENDPOINT_PATH, config.EXPERT_APP_URL).toString();
