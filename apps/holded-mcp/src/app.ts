...
// Returns tenantId if the user has an existing EXPERT connection, null otherwise.
...
export function createApp() {
    ...
    helmet({
      ...
      contentSecurityPolicy: {
        directives: {
          defaultSrc: ["'self'"],
          baseUri: ["'self'"],
          connectSrc: ["'self'", config.EXPERT_APP_URL],
          formAction: ["'self'"],
          frameAncestors: ["'none'"],
          imgSrc: ["'self'", 'data:', new URL(config.EXPERT_PUBLIC_URL).origin],
          objectSrc: ["'none'"],
          scriptSrc: ["'self'"],
  ...
  app.get('/launch', async (req, res) => {
    ...
    const bridgeUrl = new URL('/auth/holded-claude', config.EXPERT_PUBLIC_URL);
  ...
  const holdedBase = `${config.EXPERT_PUBLIC_URL.replace(/\/$/, '')}/holded/conectores/claude`;
  ...
  app.get('/.well-known/oauth-authorization-server', (_req, res) => {
    res.json({
      ...
      op_policy_uri: `${holdedBase}/privacy`,
      op_tos_uri: `${holdedBase}/terms`,
