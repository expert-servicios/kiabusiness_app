...
function connectorPublicBase() {
  return `${config.EXPERT_PUBLIC_URL.replace(/\/$/, '')}/holded/conectores/claude`;
}

// ──────────────────────────────────────────────────────────────────────────
// Optional bridge to EXPERT auth.
// ──────────────────────────────────────────────────────────────────────────
...
//      - NO → redirect 302 a EXPERT auth bridge
...
//      cookie `__session` and redirects to the MCP server.
...
const EXPERT_SESSION_COOKIE_NAME = '__session';

interface ExpertSessionPayload {
  uid: string;
  email: string;
  name?: string | null;
  tenantId?: string;
...
function readExpertSessionCookieRaw(req: Request): string | null {
  ...
  for (const c of cookies) {
    ...
    if (name === EXPERT_SESSION_COOKIE_NAME) {
      try {
...
async function verifyExpertSession(req: Request): Promise<ExpertSessionPayload | null> {
  if (!config.SESSION_SECRET) return null;
  const token = readExpertSessionCookieRaw(req);
...
function buildFirebaseBridgeUrl(req: Request, options?: { forceRelogin?: boolean }): string {
  ...
  // The MCP server is Claude-only, so the generic Holded auth form is not used.
  ...
  if (options?.forceRelogin) {
    ...
    // con sesión vieja.
    const forceUrl = new URL('/api/auth/holded-claude/force-relogin', config.EXPERT_PUBLIC_URL);
  ...
  const claudeUrl = new URL('/auth/holded-claude', config.EXPERT_PUBLIC_URL);
...
async function isClaudeConnectionActive(userId: string): Promise<boolean> {
  if (config.EXPERT_CENTRAL_REGISTRY_ENABLED !== '1') return true;
  const url = new URL('/api/integrations/holded/connection-status', config.EXPERT_APP_URL);
  url.searchParams.set('userId', userId);
  url.searchParams.set('channel', 'claude');
  const headers: Record<string, string> = { Accept: 'application/json' };
  if (config.EXPERT_APP_SHARED_SECRET) {
    headers['x-expert-shared-secret'] = config.EXPERT_APP_SHARED_SECRET;
  }
...
const F1_ERROR_MESSAGES: Record<string, string> = {
  ...
  persist_failed:
    'Error interno guardando la conexion. Si persiste, escribenos a info@expertconsulting.es.',
  central_registry_unavailable:
    'No hemos podido registrar la conexion en EXPERT. Intentalo de nuevo en unos segundos.',
};

async function probeHoldedApiKeyForStandalone(
  holdedApiKey: string
): Promise<'ok' | 'invalid_api_key' | 'probe_failed'> {
  try {
    await new HoldedClient(holdedApiKey).listContacts({ page: '1' });
    return 'ok';
  } catch (err) {
    if (
    ...
    logger.warn('Standalone Holded API key probe failed', {
      status: err instanceof HoldedApiError ? err.status : undefined,
...
async function callUpsertFromKey(input: {
  ...
  const url = new URL(F1_UPSERT_PATH, config.EXPERT_APP_URL).toString();
...
oauthRouter.post('/register', async (req: Request, res: Response) => {
  ...
  // logo antiguo de otro despliegue en lugar del diamante Holded).
  res.status(201).json({
    client_id: clientId,
    client_name: client_name ?? 'Holded',
    client_uri: connectorPublicBase(),
    ...
    policy_uri: `${connectorPublicBase()}/privacy`,
    tos_uri: `${connectorPublicBase()}/terms`,
  });
});
...
oauthRouter.get('/authorize', async (req: Request, res: Response) => {
  ...
  // de EXPERT, consistente con la landing del conector.
  //
  // Nuevo flow:
  ...
  // tiene TTL 60s y se firma con EXPERT_APP_SHARED_SECRET. Sin sesión válida o con conexión disconnected, el
  // bridge añade force_relogin=1 (ver buildFirebaseBridgeUrl).
  if (config.EXPERT_OAUTH_BRIDGE_ENABLED !== '1') {
    res
      .status(200)
      .send(
        consentPage(
  ...
  const expertSession = await verifyExpertSession(req);

  let forceRelogin = false;
  if (expertSession?.uid) {
    const active = await isClaudeConnectionActive(expertSession.uid);
    if (!active) {
      forceRelogin = true;
    }
  }
...
//
// Recibe un handoff JWT (query `handoff`) firmado con EXPERT_APP_SHARED_SECRET
...
oauthRouter.get('/finalize-from-form', async (req: Request, res: Response) => {
  ...
  if (!config.EXPERT_APP_SHARED_SECRET) {
    logger.error('[oauth/finalize-from-form] EXPERT_APP_SHARED_SECRET not configured');
  ...
  try {
    const verified = await jwtVerify(
      handoff,
      new TextEncoder().encode(config.EXPERT_APP_SHARED_SECRET),
      {
...
oauthRouter.post('/authorize', async (req: Request, res: Response) => {
  ...
  // Si hay sesión EXPERT verificada,
  ...
  const bodyEmailRaw =
    typeof req.body?.personal_email === 'string'
      ...
      : '';
  const personalEmailRaw = expertSession
    ? expertSession.email.trim().toLowerCase()
    : bodyEmailRaw;
  if (expertSession && bodyEmailRaw && bodyEmailRaw !== personalEmailRaw) {
    logger.warn(
  ...
  let upsertResult: F1UpsertSuccessPayload | null = null;
  if (config.EXPERT_CENTRAL_REGISTRY_ENABLED === '1') {
    try {
      const f1Result = await callUpsertFromKey({
      ...
      if (!f1Result.ok) {
        const reason = f1Result.reason ?? 'persist_failed';
      ...
      upsertResult = f1Result;
    } catch (err) {
      logger.error('EXPERT registry upsert network failure, refusing OAuth code', {
        message: err instanceof Error ? err.message : String(err),
      });
      res
        .status(503)
    ...
    const probeResult = await probeHoldedApiKeyForStandalone(holdedApiKey);
    if (probeResult !== 'ok') {
      const message =
        probeResult === 'invalid_api_key'
          ? F1_ERROR_MESSAGES.invalid_api_key
          : F1_ERROR_MESSAGES.probe_failed;
      res
        .status(probeResult === 'invalid_api_key' ? 400 : 503)
        .send(
          consentPage(
            authorizeContext.clientId,
            authorizeContext.redirectUri,
  ...
  const code = await createAuthorizationCode({
    ...
    userId: upsertResult?.userId ?? null,
    tenantId: upsertResult?.tenantId ?? null,
    personalEmail: personalEmailRaw,
  });
...
oauthRouter.post('/token', async (req: Request, res: Response) => {
  ...
  if (grant_type === 'authorization_code') {
    ...
    const tokenPair = await createTokenPair({
      ...
      // al token pair para que la BD del MCP refleje el User.id central cuando exista.
    ...
    <div class="card">
      ...
      <form method="POST" action="${escapedActionUrl}">
        ...
        <label class="terms-row">
          ...
          <span>Acepto los <a href="${connectorPublicBase()}/terms" target="_blank" rel="noopener">terminos de uso</a> y la <a href="${connectorPublicBase()}/privacy" target="_blank" rel="noopener">politica de privacidad</a>.</span>
        </label>
      ...
      <div class="trust">
        <span>✓ RGPD</span>
        <span>·</span>
        <a href="${connectorPublicBase()}/dpa" target="_blank" rel="noopener">DPA</a>
        <span>·</span>
      ...
      <div class="footer">
        <p class="footer-text">
          Si necesitas ayuda, escribenos a <a href="mailto:info@expertconsulting.es">info@expertconsulting.es</a>.
        </p>
      </div>
    </div>
  </div>
