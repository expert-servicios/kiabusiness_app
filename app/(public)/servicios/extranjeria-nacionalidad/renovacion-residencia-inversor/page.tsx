    ...
    <main className="bg-[#F8F6F1] text-[#0D1B2A]">
            ...
            <aside className="border border-white/12 bg-white/5 p-6 shadow-2xl shadow-black/20">
              ...
              <div className="mt-5 space-y-4">
                <div className="border border-white/10 bg-white/5 p-4">
                  <p className="text-sm text-white/60">Titular inversor</p>
                  <p className="mt-1 text-3xl font-bold text-white">250 € + IVA</p>
                </div>
                <div className="border border-white/10 bg-white/5 p-4">
                  <p className="text-sm text-white/60">Cada familiar adicional</p>
                  <p className="mt-1 text-3xl font-bold text-white">90 € + IVA</p>
                </div>
              </div>
              <p className="mt-5 text-sm leading-6 text-white/55">
                Tasas, certificados, notas simples, traducciones, apostillas, antecedentes penales, informes oficiales y otros costes externos se pagan aparte.
              </p>
            </aside>
          </div>
        </div>
      ...
      <section className="bg-white px-6 py-14 md:py-18">
        <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#D4A017]">Alcance</p>
            <h2 className="mt-3 font-serif text-3xl font-bold">Qué incluye el servicio</h2>
            <div className="mt-8 grid gap-3 sm:grid-cols-2">
              {included.map((item) => (
                <div key={item} className="flex gap-3 border border-[#eadfca] bg-[#F8F6F1] p-4">
                  <CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0 text-[#D4A017]" />
                  <p className="text-sm leading-6 text-[#29384A]">{item}</p>
                </div>
              ))}
            </div>
          </div>
          <aside className="border border-amber-200 bg-amber-50 p-6">
            <AlertTriangle className="h-6 w-6 text-amber-700" />
            <h3 className="mt-4 text-lg font-bold text-amber-950">Costes externos aparte</h3>
            <ul className="mt-4 space-y-2 text-sm leading-6 text-amber-950/80">
      ...
      <section className="px-6 py-14 md:py-18">
        <div className="mx-auto max-w-6xl">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#D4A017]">Documentación</p>
          <h2 className="mt-3 font-serif text-3xl font-bold">Checklist por bloques</h2>
          <div className="mt-8 grid gap-5 md:grid-cols-2">
            {documents.map(({ title, Icon, items }) => (
              <div key={title} className="border border-[#e4d8c1] bg-white p-6">
                <div className="flex items-center gap-3">
                  <Icon className="h-5 w-5 text-[#D4A017]" />
                  <h3 className="text-lg font-bold">{title}</h3>
                </div>
                <ul className="mt-4 space-y-2 text-sm leading-6 text-[#29384A]">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-8 grid gap-5 md:grid-cols-3">
            ...
            <div className="border border-white/10 bg-white/5 p-6">
              ...
              <FreeMeetingButton className="mt-5 inline-flex min-h-11 items-center justify-center border border-white/25 px-5 py-2.5 text-sm font-bold text-white/85 hover:border-white/60 hover:text-white">
                Reservar 15 minutos
              </FreeMeetingButton>
            </div>
          </div>
      ...
      <section className="bg-[#0D1B2A] px-6 py-14 text-white md:py-18">
        <div className="mx-auto max-w-6xl">
          ...
          <h2 className="mt-3 font-serif text-3xl font-bold">Enlaces de referencia</h2>
          <div className="mt-8 grid gap-4 md:grid-cols-2">
            <a className="border border-white/10 bg-white/5 p-5 hover:border-[#D4A017]/50" href="https://ciudadaniaexterior.inclusion.gob.es/web/unidadgrandesempresas/inversores" target="_blank" rel="noreferrer">
              <span className="flex items-center gap-2 font-bold">UGE — Inversores <ExternalLink className="h-4 w-4" /></span>
              <p className="mt-2 text-sm leading-6 text-white/60">Información oficial sobre inversores y régimen transitorio.</p>
            </a>
            <a className="border border-white/10 bg-white/5 p-5 hover:border-[#D4A017]/50" href="https://www.inclusion.gob.es/documents/1823432/1826095/renovaciones_feb_2018.pdf/7e4b1631-3f10-171a-eb21-5119133d01a5?t=1664875009918" target="_blank" rel="noreferrer">
              <span className="flex items-center gap-2 font-bold">UGE — Renovaciones Ley 14/2013 <ExternalLink className="h-4 w-4" /></span>
              <p className="mt-2 text-sm leading-6 text-white/60">Documentación orientativa oficial para renovaciones.</p>
            </a>
            <a className="border border-white/10 bg-white/5 p-5 hover:border-[#D4A017]/50" href="https://www.boe.es/buscar/act.php?id=BOE-A-2013-10074" target="_blank" rel="noreferrer">
              <span className="flex items-center gap-2 font-bold">BOE — Ley 14/2013 <ExternalLink className="h-4 w-4" /></span>
              <p className="mt-2 text-sm leading-6 text-white/60">Texto consolidado de la Ley 14/2013.</p>
            </a>
            <a className="border border-white/10 bg-white/5 p-5 hover:border-[#D4A017]/50" href="https://comercio.gob.es/es-es/inversiones_exteriores/movilidadinternacional/Paginas/default.aspx" target="_blank" rel="noreferrer">
              <span className="flex items-center gap-2 font-bold">PRIE — Ministerio de Economía <ExternalLink className="h-4 w-4" /></span>
              <p className="mt-2 text-sm leading-6 text-white/60">Programa de Residencia para Inversores y Emprendedores e informes oficiales.</p>
            </a>
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-14 md:py-18">
        <div className="mx-auto max-w-4xl">
          <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#D4A017]">Preguntas frecuentes</p>
          <div className="mt-6 divide-y divide-[#eadfca] border border-[#eadfca] bg-white">
            {faq.map((item) => (
              <details key={item.q} className="group p-5">
                <summary className="cursor-pointer text-base font-bold text-[#0D1B2A]">{item.q}</summary>
                <p className="mt-3 text-sm leading-7 text-[#29384A]">{item.a}</p>
              </details>
            ))}
          </div>
