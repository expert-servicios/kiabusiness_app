...
export default function ClientFiscalObligationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setContext(ctx);
      setObligations(fiscal.obligations ?? []);
  ...
  const create = async () => {
    if (!companyId || !title.trim() || !dueDate) {
      setError('Entidad, título y fecha de vencimiento son obligatorios.');
      return;
    }
    setSaving(true);
    setError('');
    try {
      ...
      setModelCode(''); setTitle(''); setPeriodKey(''); setDueDate(''); setNotes('');
      await load();
    } catch (saveError) {
  ...
  const setStatus = async (obligationId: string, status: Obligation['status']) => {
    ...
    try {
      ...
      setError(saveError instanceof Error ? saveError.message : 'Error al actualizar');
    } finally {
      setSaving(false);
    }
  };
    ...
    <main className="min-h-screen bg-[#f8f4eb] px-6 py-7 text-[#07111d]">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#c88b25]">Cliente 360 · Fiscal</p>
            <h1 className="mt-1 font-serif text-3xl font-bold">Obligaciones y plazos</h1>
            <p className="mt-1 text-sm text-[#52606d]">{context?.client ? `${context.client.name} · ${context.client.email}` : 'Gestión fiscal confirmada por Admin'}</p>
          </div>
          <div className="flex gap-2">
            <Link href={`/admin/clientes/${id}/operaciones`} className="rounded-xl border border-[#d8cbb5] bg-white px-4 py-2 text-xs font-bold">Operaciones</Link>
            <button onClick={() => void load()} type="button" className="rounded-xl border border-[#d8cbb5] bg-white p-2.5" title="Actualizar"><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /></button>
          </div>
        </div>

        <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-xs leading-5 text-blue-800">
          EXPERT no asigna modelos fiscales automáticamente por forma jurídica o plan. Añade aquí solo obligaciones que hayas confirmado que aplican al cliente. Al guardarlas se crea la tarea Admin correspondiente y, si está disponible, el vencimiento se sincroniza con el calendario de Admin.
        </div>

        {error && <div className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{error}</div>}

        <section className="rounded-2xl border border-[#d8cbb5] bg-white p-5">
          <h2 className="font-serif text-lg font-bold">Nueva obligación confirmada</h2>
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <label className="text-xs font-semibold">Periodo (opcional)<input value={periodKey} onChange={(e) => setPeriodKey(e.target.value)} placeholder="2026-T3" className="mt-1 w-full rounded-lg border border-[#d8cbb5] px-3 py-2 text-sm" /></label>
        ...
        <section className="rounded-2xl border border-[#d8cbb5] bg-white p-5">
          ...
          <div className="mt-4 space-y-3">
            {obligations.length === 0 ? <p className="text-sm text-[#6b7280]">Todavía no hay obligaciones fiscales confirmadas.</p> : obligations.map((item) => {
              ...
              return <article key={item.id} className="rounded-xl border border-[#eee6d8] p-4">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <div className="flex flex-wrap items-center gap-2"><CalendarClock className="h-4 w-4 text-[#c88b25]" /><h3 className="font-bold">{item.model_code ? `Modelo ${item.model_code} · ` : ''}{item.title || item.kind}</h3><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold">{statusLabel(item.status)}</span></div>
                    <p className="mt-1 text-xs text-[#52606d]">{company?.name ?? 'Entidad'} · vence {new Date(`${item.due_date}T12:00:00`).toLocaleDateString('es-ES')}{item.period_key ? ` · ${item.period_key}` : ''}</p>
                    {item.notes && <p className="mt-2 text-xs leading-5 text-[#6b7280]">{item.notes}</p>}
                    <div className="mt-2 flex flex-wrap gap-2 text-[10px] text-[#8a9aab]"><span>{item.task_id ? 'Tarea Admin vinculada' : 'Tarea pendiente de sincronización'}</span><span>{item.google_event_id ? 'Calendario Admin sincronizado' : 'Sin evento Calendar'}</span></div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {item.status === 'planned' && <button disabled={saving} onClick={() => void setStatus(item.id, 'in_progress')} className="inline-flex items-center gap-1 rounded-lg border border-[#d8cbb5] px-2.5 py-1.5 text-[11px] font-bold"><CircleDot className="h-3.5 w-3.5" />En curso</button>}
                    {(item.status === 'planned' || item.status === 'in_progress') && <button disabled={saving} onClick={() => void setStatus(item.id, 'completed')} className="inline-flex items-center gap-1 rounded-lg border border-emerald-200 bg-emerald-50 px-2.5 py-1.5 text-[11px] font-bold text-emerald-800"><CheckCircle2 className="h-3.5 w-3.5" />Completar</button>}
                    {(item.status === 'planned' || item.status === 'in_progress') && <button disabled={saving} onClick={() => void setStatus(item.id, 'cancelled')} className="inline-flex items-center gap-1 rounded-lg border border-red-200 bg-red-50 px-2.5 py-1.5 text-[11px] font-bold text-red-700"><XCircle className="h-3.5 w-3.5" />Cancelar</button>}
                  </div>
                </div>
              </article>;
            })}
