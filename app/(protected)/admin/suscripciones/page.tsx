        ...
        <div className="rounded-3xl border border-[#d8cbb5] bg-white p-8 shadow-lg">
          ...
          {contextual && (
            <section className="mb-8 rounded-2xl border border-[#e6dfd2] bg-[#faf8f2] p-5">
              <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-[#c88b25]">Intentos de Checkout</p><p className="mt-1 text-sm text-[#29384a]">Historial registrado en EXPERT. No se crea, cancela ni modifica ninguna sesión desde esta lista.</p></div><div className="flex flex-wrap gap-2">{params.clientId && <Link href={`/admin/clientes/${params.clientId}/beneficios`} className="inline-flex items-center gap-1.5 rounded-xl border border-[#d8cbb5] bg-white px-4 py-2 text-xs font-bold text-[#07111d]"><Gift className="h-3.5 w-3.5" />Beneficios</Link>}{params.clientId && <Link href={`/admin/suscripciones/generar?clientId=${params.clientId}`} className="rounded-xl bg-[#07111d] px-4 py-2 text-xs font-bold text-white">Contratación / Checkout</Link>}</div></div>
              {checkouts.length === 0 ? <p className="mt-4 text-sm text-[#6b7280]">No hay Checkout registrados para este contexto.</p> : <div className="mt-4 space-y-3">{checkouts.map((checkout) => {
                ...
                return <div key={checkout.id} className="rounded-xl border border-[#d8cbb5] bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-[#c88b25]" /><p className="text-sm font-bold text-[#07111d]">{plan}{amount ? ` · ${amount} €/mes + IVA` : ''}</p></div><p className="mt-1 font-mono text-[10px] text-[#6b7280]">{checkout.stripe_session_id}</p><p className="mt-1 text-xs text-[#6b7280]">Creado {new Date(checkout.created_at).toLocaleString('es-ES')} · Email EXPERT: {emailSent ? 'registrado' : 'sin registro'}</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${isOpen ? 'bg-amber-100 text-amber-800' : checkout.status === 'complete' || checkout.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700'}`}>{checkout.status}</span></div>{isOpen && params.clientId && <div className="mt-3 flex flex-wrap gap-2"><Link href={`/admin/suscripciones/generar?clientId=${params.clientId}`} className="inline-flex items-center gap-1.5 rounded-lg border border-[#d8cbb5] px-3 py-2 text-xs font-bold text-[#29384a]"><ExternalLink className="h-3.5 w-3.5" />Gestionar Checkout abierto</Link><Link href={`/admin/clientes/${params.clientId}/beneficios`} className="rounded-lg border border-[#d8cbb5] px-3 py-2 text-xs font-bold text-[#29384a]">Añadir beneficio</Link><Link href={`/admin/clientes/${params.clientId}/comunicaciones`} className="rounded-lg border border-[#d8cbb5] px-3 py-2 text-xs font-bold text-[#29384a]">Revisar comunicaciones</Link></div>}</div>;
              })}</div>}
            </section>
          )}
