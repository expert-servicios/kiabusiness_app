...
export default function AdminGenerateSubscriptionLinkPage() {
  ...
  async function chooseClient(id: string) {
    setSelectedClientId(id); setDetail(null); setCompanyId(''); setResult(null); setError(''); setLoadingDetail(true);
    try {
  ...
  async function copyLink() { if (result?.stripeUrl) await navigator.clipboard.writeText(result.stripeUrl); }
  ...
  return (
    <main className="min-h-screen bg-[#f8f4eb] px-6 py-8 text-[#07111d]"><div className="mx-auto max-w-4xl">
      <div className="flex items-start gap-3">
        ...
        <div><p className="text-xs font-bold uppercase tracking-[0.2em] text-[#c88b25]">Admin · Suscripciones</p><h1 className="mt-1 font-serif text-3xl font-bold">Preparar contratación</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#29384a]">Crea o reutiliza la trazabilidad comercial canónica de EXPERT (lead → presupuesto → expediente → Checkout) para una entidad concreta. El enlace no se envía automáticamente.</p></div>
      </div>
      ...
      {detail && <section className="mt-5 rounded-2xl border border-[#d8cbb5] bg-white p-6 shadow-sm">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2"><label className="text-sm font-semibold">2. Entidad contratante<select value={companyId} onChange={(e) => setCompanyId(e.target.value)} className="mt-2 w-full rounded-xl border border-[#d8cbb5] bg-white px-3 py-3 text-sm"><option value="">Selecciona entidad</option>{detail.companies.map((company) => <option key={company.id} value={company.id}>{company.razon_social ?? company.name} {company.nif ? `· ${company.nif}` : ''}</option>)}</select></label><label className="text-sm font-semibold">3. Plan<select value={planKey} onChange={(e) => setPlanKey(e.target.value as (typeof PLANS)[number]['key'])} className="mt-2 w-full rounded-xl border border-[#d8cbb5] bg-white px-3 py-3 text-sm">{PLANS.map((plan) => <option key={plan.key} value={plan.key}>{plan.name} · {plan.amount} €/mes + IVA</option>)}</select></label></div>
      ...
      {result && <section className="mt-5 rounded-2xl border border-green-200 bg-green-50 p-6"><div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 text-green-700" /><div className="min-w-0 flex-1"><h2 className="font-semibold text-green-900">Contratación preparada y Checkout registrado</h2><p className="mt-1 text-xs text-green-800">Session ID: <span className="font-mono">{result.sessionId}</span></p>{result.quoteId && <p className="mt-1 text-xs text-green-800">Presupuesto: <span className="font-mono">{result.quoteId}</span></p>}{result.onboardingCaseId && <p className="mt-1 text-xs text-green-800">Expediente de alta: <span className="font-mono">{result.onboardingCaseId}</span></p>}<p className="mt-3 break-all rounded-lg bg-white px-3 py-2 text-xs text-[#29384a]">{result.stripeUrl}</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => void copyLink()} className="inline-flex items-center gap-1.5 rounded-lg border border-green-300 bg-white px-3 py-2 text-xs font-semibold text-green-900"><Copy className="h-3.5 w-3.5" /> Copiar enlace</button><a href={result.stripeUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg border border-green-300 bg-white px-3 py-2 text-xs font-semibold text-green-900"><ExternalLink className="h-3.5 w-3.5" /> Verificar Checkout</a><Link href={`/admin/clientes/${selectedClientId}`} className="inline-flex items-center gap-1.5 rounded-lg border border-green-300 bg-white px-3 py-2 text-xs font-semibold text-green-900">Volver al Cliente 360</Link></div></div></div></section>}
    </div></main>
  );
}
