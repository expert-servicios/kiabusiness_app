...
import {
  ArrowLeft, CheckCircle2, Copy, ExternalLink, Eye, Loader2, Mail,
...
export default function AdminGenerateSubscriptionLinkPage() {
  ...
  async function improveWithKia() {
    ...
    try {
      ...
      setNotice('KIA ha mejorado el borrador. Revísalo antes de enviar.');
    } catch (err) {
  ...
  async function sendEmail() {
    ...
    const confirmed = window.confirm(`Se enviará este correo a ${detail?.profile.email ?? 'el cliente'} usando la plantilla EXPERT y el Checkout verificado. ¿Continuar?`);
    if (!confirmed) return;
    ...
    try {
      ...
      setEmailSent(true);
      setNotice(json.sent === false
        ? 'El correo ya había sido aceptado anteriormente; EXPERT ha bloqueado el envío duplicado.'
        : 'Correo enviado correctamente desde EXPERT.');
    } catch (err) {
      ...
      <div className="mx-auto max-w-5xl">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h1 className="mt-1 font-serif text-3xl font-bold">Contratación y correo al cliente</h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-[#29384a]">
              Genera una única sesión Stripe, revisa el correo predefinido, edítalo manualmente o mejóralo con KIA y envíalo desde EXPERT con plantilla HTML profesional.
            </p>
          </div>
        </div>
            ...
            <div className="mt-3 max-h-64 overflow-y-auto rounded-xl border border-[#f0e8d8]">
              {filteredClients.map((client) => (
                <button key={client.id} type="button" onClick={() => void chooseClient(client.id)} className={`flex w-full items-center justify-between border-b border-[#f8f4eb] px-4 py-3 text-left last:border-b-0 ${selectedClientId === client.id ? 'bg-[#d7a33a]/10' : 'hover:bg-[#f8f4eb]'}`}>
                  <span><span className="block text-sm font-semibold">{client.full_name ?? 'Sin nombre'}</span><span className="block text-xs text-[#6b7280]">{client.email}</span></span>
        ...
        {detail && (
          <section className="mt-5 rounded-2xl border border-[#d8cbb5] bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div><h2 className="font-serif text-xl font-bold">{detail.profile.full_name ?? detail.profile.email}</h2><p className="text-sm text-[#29384a]">{detail.profile.email}</p></div>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <label className="text-sm font-semibold">2. Entidad contratante
                <select value={companyId} onChange={(e) => { setCompanyId(e.target.value); setResult(null); setEmailSent(false); }} className="mt-2 w-full rounded-xl border border-[#d8cbb5] bg-white px-3 py-3 text-sm">
                  <option value="">Selecciona entidad</option>
                  {detail.companies.map((company) => <option key={company.id} value={company.id}>{company.razon_social ?? company.name} {company.nif ? `· ${company.nif}` : ''}</option>)}
                </select>
              </label>

              <label className="text-sm font-semibold">3. Plan
                <select value={planKey} onChange={(e) => { setPlanKey(e.target.value as (typeof PLANS)[number]['key']); setResult(null); setEmailSent(false); }} className="mt-2 w-full rounded-xl border border-[#d8cbb5] bg-white px-3 py-3 text-sm">
                  {PLANS.map((plan) => <option key={plan.key} value={plan.key}>{plan.name} · {plan.amount} €/mes + IVA</option>)}
                </select>
              </label>
            </div>

            {activeForCompany && <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">Esta entidad ya tiene una suscripción activa/trialing. No generes otro Checkout sin revisar el caso.</div>}
            ...
            <div className="mt-6 flex justify-end">
              <button type="button" onClick={() => void generateLink()} disabled={generating || !companyId || !detail.profile.profile_completed || !detail.profile.billing_ready || activeForCompany} className="inline-flex items-center gap-2 rounded-xl bg-[#d7a33a] px-5 py-3 text-sm font-bold text-[#07111d] disabled:cursor-not-allowed disabled:opacity-50">
                {generating && <Loader2 className="h-4 w-4 animate-spin" />} Generar enlace sin enviar
              </button>
            </div>
          </section>
        )}
        ...
        {result && (
          <>
            <section className="mt-5 rounded-2xl border border-green-200 bg-green-50 p-6">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <h2 className="font-semibold text-green-900">Checkout generado y registrado</h2>
            ...
            <section className="mt-5 rounded-2xl border border-[#d8cbb5] bg-white p-6 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div><p className="text-xs font-bold uppercase tracking-wide text-[#c88b25]">4. Comunicación</p><h2 className="mt-1 font-serif text-xl font-bold">Correo de contratación</h2></div>
                <div className="flex flex-wrap gap-2">
                  <button type="button" onClick={() => setShowPreview((value) => !value)} className="inline-flex items-center gap-1.5 rounded-xl border border-[#d8cbb5] px-3 py-2 text-xs font-semibold text-[#29384a]"><Eye className="h-3.5 w-3.5" /> {showPreview ? 'Ocultar vista previa' : 'Vista previa'}</button>
                  <button type="button" onClick={() => void improveWithKia()} disabled={composing || sending || emailSent} className="inline-flex items-center gap-1.5 rounded-xl border border-[#c88b25] px-3 py-2 text-xs font-bold text-[#8b5a0a] disabled:opacity-50">
                    {composing ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Sparkles className="h-3.5 w-3.5" />} Mejorar con KIA
                  </button>
                </div>
              </div>
              ...
              <div className="mt-5 grid gap-5 lg:grid-cols-2">
                <div className="space-y-4">
                  <label className="block text-sm font-semibold">Asunto
                    <input value={emailSubject} onChange={(e) => setEmailSubject(e.target.value)} disabled={emailSent} className="mt-2 w-full rounded-xl border border-[#d8cbb5] px-3 py-3 text-sm outline-none focus:border-[#c88b25] disabled:bg-gray-50" />
                  </label>
                  <label className="block text-sm font-semibold">Mensaje
                    <textarea value={emailBody} onChange={(e) => setEmailBody(e.target.value)} disabled={emailSent} rows={15} className="mt-2 w-full resize-y rounded-xl border border-[#d8cbb5] px-3 py-3 text-sm leading-6 outline-none focus:border-[#c88b25] disabled:bg-gray-50" />
                  </label>
                  <p className="text-xs leading-5 text-[#6b7280]">El enlace Stripe no forma parte del texto editable: EXPERT lo inserta en servidor como botón seguro. KIA tampoco puede modificarlo.</p>
                </div>

                {showPreview && (
                  <div className="rounded-2xl bg-[#f8f4eb] p-4">
                    <div className="overflow-hidden rounded-2xl border border-[#d8cbb5] bg-white">
                      <div className="bg-[#07111d] px-6 py-6 text-center"><div className="font-serif text-2xl font-bold tracking-[0.2em] text-[#d7a33a]">EXPERT</div><div className="mt-1 text-[10px] uppercase tracking-[0.2em] text-[#8899aa]">Asesoría Legal · Fiscal · Administrativa</div></div>
                      <div className="p-6"><h3 className="font-serif text-xl font-bold">{selectedPlan.name}</h3><div className="mt-4 whitespace-pre-wrap text-sm leading-6 text-[#29384a]">{emailBody}</div><div className="mt-5 rounded-xl bg-[#f8f4eb] p-3 text-sm"><strong>{selectedPlan.amount} €/mes + IVA</strong></div><div className="mt-6 text-center"><span className="inline-block rounded-full bg-[#c88b25] px-6 py-3 text-xs font-bold uppercase tracking-wide text-[#07111d]">Activar {selectedPlan.name}</span></div></div>
                      <div className="border-t border-[#d8cbb5] bg-[#f8f4eb] px-5 py-4 text-center text-[10px] text-[#6b7280]">EXPERT ESTUDIOS PROFESIONALES, SLU · info@expertconsulting.es</div>
                    </div>
                  </div>
                )}
              </div>

              <div className="mt-6 flex items-center justify-between gap-4 border-t border-[#f0e8d8] pt-5">
                <div className="flex items-center gap-2 text-xs text-[#6b7280]"><Mail className="h-4 w-4" /> Destinatario: <strong className="text-[#29384a]">{detail?.profile.email}</strong></div>
                <button type="button" onClick={() => void sendEmail()} disabled={sending || composing || emailSent || !emailSubject.trim() || emailBody.trim().length < 20} className="inline-flex items-center gap-2 rounded-xl bg-[#07111d] px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">
                  {sending ? <Loader2 className="h-4 w-4 animate-spin" /> : emailSent ? <CheckCircle2 className="h-4 w-4" /> : <Send className="h-4 w-4" />}
                  {emailSent ? 'Correo enviado' : 'Revisar y enviar'}
                </button>
              </div>
            </section>
          </>
