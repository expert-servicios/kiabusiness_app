...
interface CalPayload {
  uid: string; title: string; startTime: string; endTime: string; status: string;
  attendees?: CalAttendee[]; videoCallUrl?: string | null; cancellationReason?: string | null;
  rescheduled?: boolean; previousStartTime?: string | null;
  eventType?: { title?: string; slug?: string } | null; organizer?: { email?: string } | null;
...
function meetingLabels(startTime: string, timeZone = 'Europe/Madrid') {
  const start = new Date(startTime);
  const safeZone = timeZone || 'Europe/Madrid';
  try {
    return {
      date: new Intl.DateTimeFormat('es-ES', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: safeZone }).format(start),
      time: new Intl.DateTimeFormat('es-ES', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: safeZone }).format(start),
    };
  } catch {
    return {
      date: new Intl.DateTimeFormat('es-ES', { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Europe/Madrid' }).format(start),
      time: new Intl.DateTimeFormat('es-ES', { hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'Europe/Madrid' }).format(start),
    };
  }
}

async function sendOnboardingPreparation(attendee: CalAttendee, payload: CalPayload) {
  const labels = meetingLabels(payload.startTime, attendee.timeZone ?? 'Europe/Madrid');
  const tpl = onboardingPreparationEmail({ name: attendee.name, meetingDate: labels.date, meetingTime: labels.time, meetingUrl: payload.videoCallUrl ?? null });
  await sendEmail({
    to: attendee.email,
    eventType: 'onboarding.preparation',
    ...tpl,
    metadata: { cal_uid: payload.uid, slug: 'onboarding', onboarding_phase: 'pre_meeting', start_time: payload.startTime },
    idempotencyKey: `cal/onboarding-preparation/${payload.uid}/${payload.startTime}`.slice(0, 256),
  });
}
...
async function notifyAdminBooking(attendee: CalAttendee, payload: CalPayload, slug: string) {
  ...
  await sendEmail({ to: adminEmails, eventType: 'onboarding.booking.admin', subject: `Reserva ${service} — ${attendee.name}`, html: `<p>Nueva reserva administrativa registrada en EXPERT.</p><p><strong>Cliente:</strong> ${attendee.name} (${attendee.email})</p><p><strong>Servicio:</strong> ${service}</p><p><strong>Inicio:</strong> ${payload.startTime}</p>${meeting}`, metadata: { cal_uid: payload.uid, slug, client_email: attendee.email }, idempotencyKey: `cal/admin-booking/${payload.uid}` });
}
  ...
  try {
    if (triggerEvent === 'BOOKING_CREATED') {
      ...
      const { data: appointment, error: upsertError } = await admin.from('appointments').upsert({
        cal_uid: payload.uid, name: attendee?.name ?? '', email: attendee?.email ?? '', phone, appointment_type: slug || 'cal_booking', appointment_date: payload.startTime,
        service: payload.eventType?.title ?? payload.title, status: 'confirmed', confirmed_date: confirmedDate, confirmed_time: confirmedTime, meeting_url: meetingUrl, notes: null, updated_at: new Date().toISOString(),
      ...
      if (attendee?.email && ['reunion', 'demo', 'academy-admision', 'onboarding', 'formacion'].includes(slug)) {
        const labels = meetingLabels(payload.startTime, attendee.timeZone ?? 'Europe/Madrid');
        sendEmail({ to: attendee.email, eventType: slug === 'onboarding' ? 'onboarding.booking.confirmed' : 'cita.confirmed', ...citaConfirmed(attendee.name, payload.eventType?.title ?? payload.title, labels.date, labels.time, meetingUrl), metadata: { cal_uid: payload.uid, slug }, idempotencyKey: `cal/client-confirmation/${payload.uid}` }).catch((e: unknown) => console.error('[cal/webhook] citaConfirmed email:', e));
      }
