  ...
  try {
    if (triggerEvent === 'BOOKING_CREATED') {
      ...
      if (attendee?.email && ['reunion', 'demo', 'academy-admision', 'onboarding', 'formacion'].includes(slug)) {
        ...
        if (slug === 'onboarding') {
          const prep = onboardingPreparationEmail({
            name: attendee.name,
            meetingDate: dateLabel,
            meetingTime: confirmedTime,
            meetingUrl,
          });
          sendEmail({
            to: attendee.email,
            ...
            ...prep,
            metadata: { cal_uid: payload.uid, slug, meeting_date: confirmedDate, meeting_time: confirmedTime },
            idempotencyKey: `cal/onboarding-preparation/${payload.uid}`,
          }).catch((e: unknown) => console.error('[cal/webhook] onboardingPreparation email:', e));
        }
      }
