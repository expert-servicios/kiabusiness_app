...
async function loadCheckout(sessionId: string) {
  ...
  const [{ data: profile }, { data: company }, { data: activeSubscription }] = await Promise.all([
  ...
  if (!profile?.email) return { error: 'No se pudo resolver el email del cliente', status: 500 as const };
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = raw?.action === 'compose' ? composeSchema.safeParse(raw) : sendSchema.safeParse(raw);
    if (!parsed.success) {
    ...
    const clientName = checkout.profile.full_name ?? checkout.profile.email.split('@')[0];
    ...
    const delivery = await sendEmailOnce({
      ...
      metadata: {
        ...
        source: 'admin_subscription_composer',
      },
    });
    ...
    const nextMetadata = {
      ...
      email_source: 'admin_subscription_composer',
    };
    await checkout.admin
      .from('checkout_sessions')
      .update({ metadata: nextMetadata, updated_at: new Date().toISOString() })
      .eq('stripe_session_id', checkout.local.stripe_session_id);
    ...
    await checkout.admin.from('audit_logs').insert({
      ...
      metadata: {
        client_id: checkout.local.user_id,
        client_email: checkout.profile.email,
        company_id: checkout.local.company_id,
        plan_name: checkout.planName,
        session_id: checkout.local.stripe_session_id,
    ...
    return NextResponse.json({ ok: true, sent: delivery.sent, resendId: delivery.resendId });
  } catch (error) {
