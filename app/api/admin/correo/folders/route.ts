...
export async function GET(request: NextRequest) {
  ...
  const [{ data: folders, error: foldersError }, { data: states, error: statesError }] = await Promise.all([
    admin
  ...
  if (foldersError || statesError) {
    console.error('[admin/correo/folders] read failed', foldersError ?? statesError);
  ...
  for (const state of states ?? []) {
    if (state.folder_id && !state.is_archived) {
      counts.set(state.folder_id, (counts.get(state.folder_id) ?? 0) + 1);
    }
  }
