...
describe('current mutable search-path hardening', () => {
  it('pins the four functions confirmed by production preflight', () => {
    expect(migration).toContain('alter function app.assign_master_admin() set search_path = app, public, pg_temp;');
    expect(migration).toContain('alter function public.increment_helpful_count(uuid) set search_path = public, pg_temp;');
    expect(migration).toContain('alter function public.is_admin_email() set search_path = public, pg_temp;');
    expect(migration).toContain('alter function public.is_admin_user() set search_path = public, pg_temp;');
  });
