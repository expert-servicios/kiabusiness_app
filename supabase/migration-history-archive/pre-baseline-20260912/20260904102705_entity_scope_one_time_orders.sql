...
for each row execute function public.inherit_checkout_company_to_order();
...
comment on function public.inherit_checkout_company_to_order() is 'For new orders only, inherit contracting company_id from persisted Stripe checkout context.';
comment on function public.complete_checkout_session_from_order() is 'For new orders only, mark the matching persisted Stripe checkout session completed.';
