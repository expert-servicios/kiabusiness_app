...
create unique index if not exists holded_contact_creation_claims_active_company_uidx on public.holded_contact_creation_claims(company_id) where state in ('claimed','creating','completed','manual_review');
...
returns void language plpgsql security definer set search_path=public,pg_temp as $$ begin update public.holded_contact_creation_claims set state='creating',lease_expires_at=now(),updated_at=now() where id=p_claim_id and owner_token=p_owner_token and state='claimed'; if not found then raise exception 'Holded contact claim ownership lost'; end if; end; $$;
