...
create unique index if not exists subscription_checkout_claims_active_intent_uidx on public.subscription_checkout_claims(user_id, company_id, intent_key) where state in ('claimed','open','manual_review');
create unique index if not exists subscription_checkout_claims_session_uidx on public.subscription_checkout_claims(stripe_session_id) where stripe_session_id is not null;
create index if not exists subscription_checkout_claims_lease_idx on public.subscription_checkout_claims(lease_expires_at) where state = 'claimed';
...
create or replace function public.expire_subscription_checkout_claim(p_claim_id uuid,p_owner_token uuid,p_error text default null)
returns void language plpgsql security definer set search_path=public,pg_temp as $$
begin
...
comment on table public.subscription_checkout_claims is 'Atomic pre-Stripe claims for subscription checkout creation. No historical billing data is stored or rewritten here.';
